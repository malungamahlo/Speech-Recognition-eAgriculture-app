...
import com.Farm.e_agric.CommentsActivity;
import com.Farm.e_agric.Model.Post;
...
public class PostAdapter  extends RecyclerView.Adapter<PostAdapter.PostViewHolder> {
    ...
    public void onBindViewHolder(@NonNull PostViewHolder holder, int position) {
        ...
        holder.setPostCaption(post.getQuestion());
        ...
        holder.likePic.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                firestore.collection("Posts/" + postId + "/critical").document(currentUserId).get().addOnCompleteListener(new OnCompleteListener<DocumentSnapshot>() {
                    @Override
                    public void onComplete(@NonNull Task<DocumentSnapshot> task) {
                       if (!task.getResult().exists()){
                           Map<String , Object> likesMap = new HashMap<>();
                           likesMap.put("timestamp" , FieldValue.serverTimestamp());
                           firestore.collection("Posts/" + postId + "/critical").document(currentUserId).set(likesMap);
                       }else{
                           firestore.collection("Posts/" + postId + "/critical").document(currentUserId).delete();
                       }
                    }
                });
            }
        ...
        //like color change
        firestore.collection("Posts/" + postId + "/critical").document(currentUserId).addSnapshotListener(new EventListener<DocumentSnapshot>() {
            @Override
            public void onEvent(@Nullable DocumentSnapshot value, @Nullable FirebaseFirestoreException error) {
                 if (error == null){
                     if (value.exists()){
                         holder.likePic.setImageDrawable(context.getDrawable(R.drawable.criticalafter));
                     }else{
                         holder.likePic.setImageDrawable(context.getDrawable(R.drawable.critical));
                     }
                 }
            }
        });
        ...
        //likes count
        firestore.collection("Posts/" + postId + "/critical").addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
                    ...
                    alert.setTitle("Delete")
                            ...
                            .setPositiveButton("Yes", new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int which) {
                                    ...
                                    firestore.collection("Posts/" + postId + "/solutions").get()
                                            .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                                                @Override
                                                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                                 for (QueryDocumentSnapshot snapshot : task.getResult()){
                                                     firestore.collection("Posts/" + postId + "/solutions").document(snapshot.getId()).delete();
                                                 }
                                                }
                                            });
                                    firestore.collection("Posts/" + postId + "/critical").get()
                                            .addOnCompleteListener(new OnCompleteListener<QuerySnapshot>() {
                                                @Override
                                                public void onComplete(@NonNull Task<QuerySnapshot> task) {
                                                    for (QueryDocumentSnapshot snapshot : task.getResult()){
                                                        firestore.collection("Posts/" + postId + "/critical").document(snapshot.getId()).delete();
                                                    }
                                                }
                                            });
    ...
    public class PostViewHolder extends RecyclerView.ViewHolder{
        ...
        public void setPostLikes(int count){
            ...
            postLikes.setText(count + "/10 critical"); // change to Likes
        }
